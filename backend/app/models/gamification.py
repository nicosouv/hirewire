...
class Achievement(Base):
    ...
    )  # applications, interviews, streak, special
    ...
    )  # {type: 'count', target: 10, metric: 'applications'}
...
class UserAchievement(Base):
    ...
    achievement_id = Column(
        Integer,
        ForeignKey("hirewire.achievements.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class ActivityLog(Base):
    ...
    activity_type = Column(
        String(50), nullable=False
    )  # application, interview, profile_update, login
