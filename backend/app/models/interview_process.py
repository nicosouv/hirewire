...
class InterviewProcess(Base):
    ...
    user_id = Column(
        Integer, ForeignKey("hirewire.users.id"), nullable=False, index=True
    )
    job_position_id = Column(
        Integer, ForeignKey("hirewire.job_positions.id"), nullable=False, index=True
    )
    ...
    interviews = relationship(
        "Interview", back_populates="process", cascade="all, delete-orphan"
    )
