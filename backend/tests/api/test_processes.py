...
class TestProcessEndpoints:
    ...
    def test_list_processes(self, client: TestClient, test_process: InterviewProcess):
    ...
    def test_get_process(self, client: TestClient, test_process: InterviewProcess):
        """Test getting a single process"""
        response = client.get(f"/api/v1/processes/{test_process.id}")
    ...
    def test_create_process(self, client: TestClient, test_position: JobPosition):
    ...
    def test_update_process_status(self, client: TestClient, test_process: InterviewProcess):
        """Test updating process status"""
        update_data = {"status": "interviewing"}

        response = client.put(f"/api/v1/processes/{test_process.id}", json=update_data)

        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "interviewing"

    def test_delete_process(self, client: TestClient, test_process: InterviewProcess, db: Session):
        """Test deleting a process"""
        response = client.delete(f"/api/v1/processes/{test_process.id}")
    ...
    def test_filter_processes_by_status(
        self, client: TestClient, test_position: JobPosition, db: Session
    ):
        ...
        # Filter by status
        response = client.get("/api/v1/processes?status=interviewing")
    ...
    def test_process_status_transitions(
        self, client: TestClient, test_process: InterviewProcess
    ):
        ...
        for from_status, to_status in valid_transitions:
            ...
            client.put(
                f"/api/v1/processes/{test_process.id}", json={"status": from_status}
            )

            # Transition to to_status
            response = client.put(
                f"/api/v1/processes/{test_process.id}", json={"status": to_status}
            )

            assert response.status_code == 200
            assert response.json()["status"] == to_status

    def test_process_requires_position(self, client: TestClient):
    ...
    def test_process_date_validation(self, client: TestClient, test_position: JobPosition):
    ...
    def test_get_active_processes(
        ...
        response = client.get("/api/v1/processes?active=true")
